...
func (m *mockStore) RecomputeReplyEdges(_ context.Context, since time.Time, limit int) error {
	m.gotSince, m.gotLimit = since, limit
	return m.record("reply_edges")
}
func (m *mockStore) RecomputeEngagementReal(_ context.Context, _ time.Time, _ int, th clickhouse.Thresholds, _ time.Time) error {
	m.gotThVer = th.Version
	return m.record("engagement_real")
}
func (m *mockStore) RecomputeUserStats(_ context.Context, _ time.Time, _ int, _ time.Time) error {
	return m.record("user_stats")
}
...
func TestRunOnce_RunsStagesInDependencyOrder(t *testing.T) {
	...
	want := []string{"reply_edges", "engagement_real", "user_stats", "rank_features", "save_state"}
...
func TestRunOnce_AbortsOnStageError(t *testing.T) {
	...
	for _, c := range m.calls {
		if c == "user_stats" || c == "rank_features" || c == "save_state" {
