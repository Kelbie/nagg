...
CMD ["sh", "-c", "case \"${NAGG_PROCESS:-api}\" in api) exec ./nagg-api ;; ingester) exec ./nagg-ingester ;; enricher) exec ./nagg-enricher ;; migrate) exec ./nagg-migrate ;; backfill) exec ./nagg-backfill ;; *) echo \"unknown NAGG_PROCESS=${NAGG_PROCESS}\" >&2; exit 1 ;; esac"]
